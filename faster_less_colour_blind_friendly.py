...
def update_graph(selected_dungeon, selected_region, selected_role):
    fig = go.Figure()
...
def update_class_histogram(clickData, selected_role, selected_region, hoverData, hover_mode):
    if not clickData:
    ...
    if hoverData and 'points' in hoverData and hoverData['points']:
        hovered_spec = hoverData['points'][0].get('customdata', None)
        hovered_level = hoverData['points'][0].get('x', None)
    ...
    for i, dungeon in enumerate(dungeons):
        ...
        for spec in dungeon_data['player_spec'].unique():
            ...
            for lvl, val in zip(x_values, y_values):
                is_hovered_bar = (hovered_spec is not None and spec == hovered_spec and lvl == hovered_level)

                if hovered_spec is None:
                    # No hover: all full opacity
                    bar_colors.append(hex_to_rgba(base_hex_color, 1.0))
                else:
                    if hover_mode == 'all':
                        # Highlight all bars of hovered_spec, dim others
                        if spec == hovered_spec:
                            bar_colors.append(hex_to_rgba(base_hex_color, 1.0))
                        else:
                            bar_colors.append(hex_to_rgba(base_hex_color, 0.3))
                    else:  # hover_mode == 'specific'
                        # Only hovered_spec & hovered_level full opacity
                        if is_hovered_bar:
                            bar_colors.append(hex_to_rgba(base_hex_color, 1.0))
                        else:
                            bar_colors.append(hex_to_rgba(base_hex_color, 0.3))
            ...
            for _, row_data in spec_data.iterrows():
                ...
                top_of_segment = row_data['cumulative_percentage']  # top of this bar segment
                is_hovered_bar = (hovered_spec is not None and spec == hovered_spec and current_level == hovered_level)
                ...
                if segment_height >= bar_width:
                    if hovered_spec is None:
                        ...
                        # Only add icons if is_hovered_bar
                        if is_hovered_bar:
                            add_icon = True
                ...
                if is_hovered_bar:
                    ...
                    fig.add_annotation(
                        x=current_level,
                        y=top_of_segment + 0.5,  # 0.5% above the top segment
                        xref=xref,
                        yref=yref,
                        text=f"{val:.2f}%",
                        showarrow=False,
